...
namespace OceanBank
{
    class ChooseAcctToTopUpCashCardState : State
    {
        //Timer
        Timer aTimer;
        int counter = 30;
        ...
        public ChooseAcctToTopUpCashCardState(GUIforATM mainForm, string language) : base (mainForm, language)
        {
            aTimer = new Timer();
            ...
            {
                bigDisplayLBL.Text = "Sila pilih akaun untuk buat nilai tambahan";
            ...
            {
                bigDisplayLBL.Text = "Please select an account to Top up from";
                smallDisplayLBL.Text = "";
                left1BTN.Text = ""; left2BTN.Text = ""; left3BTN.Text = ""; left4BTN.Text = "";
        ...
        public override State handleLeft1BTNClick()
        {
            ...
            if (session)
            {
                if (left1BTN.Text != "")
                {
                    nextStep = new EnterAmtToTopupCashcardState(mainForm, language, left1BTN.Text);
        ...
        public override State handleLeft2BTNClick()
        {
            ...
            if (session)
            {
                if (left2BTN.Text != "")
                {
                    nextStep = new EnterAmtToTopupCashcardState(mainForm, language, left2BTN.Text);
                }
            }
            else
                theCardReader.ejectCard();
        ...
        public override State handleLeft3BTNClick()
        {
            ...
            if (session)
            {
                if (left3BTN.Text != "")
                {
                    nextStep = new EnterAmtToTopupCashcardState(mainForm, language, left3BTN.Text);
                }
            }
            else
                theCardReader.ejectCard();
